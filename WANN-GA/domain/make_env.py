...
def make_env(env_name, seed=-1, render_mode=False):
  ...
  # -- Classification ------------------------------------------------ -- #
    ...
    if env_name.endswith("mnist256"):
      ...
      # ------------------------------------------------
      # Also import the EMNIST 10Letters dataset for joint evaluation
      from domain.classify_gym import emnist
      trainSet, target  = mnist_256()
      secondTrain, secondTarget = emnist()
      # ------------------------------------------------

    env = ClassifyEnv(trainSet,target,secondTrain,secondTarget)  

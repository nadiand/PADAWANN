...
class Ind():
  ...
  def __init__(self, conn, node):
    ...
    self.species = []
    # ------------------------------------------------
    # Add an attribute for the accuracy
    self.accuracy = 0
    # ------------------------------------------------
...
def act(weights, aVec, nInput, nOutput, inPattern, inPattern2):
  ...
  # ------------------------------------------------
  # Check out how many are accurately predicted
  ...
  # Do everything from above but for the second dataset
  if np.ndim(inPattern2) > 1:
      nSamples = np.shape(inPattern2)[0]
  else:
      nSamples = 1
  ...
  for o in output:
    ...
    for i in range(len(o)):
      if o[i] == pred:
        ...
        break
  # ------------------------------------------------
  return output, np.array(predicted), np.array(predicted2)
